...
def test_hill_climb_max_iters():
    ...
    problem = DiscreteOpt(5, OneMax(), maximize=True)
    x = np.zeros(5)
    best_state, best_fitness, _ = hill_climb(problem, max_iters=1, restarts=0, init_state=x)
...
def test_random_hill_climb_discrete_max():
    ...
    problem = DiscreteOpt(5, OneMax(), maximize=True)
    best_state, best_fitness, _ = random_hill_climb(problem, max_attempts=10, restarts=20)
    x = np.ones(5)
    assert np.array_equal(best_state, x) and best_fitness == 5
...
def test_random_hill_climb_max_iters():
    ...
    problem = DiscreteOpt(5, OneMax(), maximize=True)
    x = np.zeros(5)
    best_state, best_fitness, _ = random_hill_climb(problem, max_attempts=1, max_iters=1, restarts=0, init_state=x)
    assert best_fitness == 1
